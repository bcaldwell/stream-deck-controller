...
async fn main() {
    ...
    let light_opt = match hue.lights().await {
        Ok(lights) => lights
            .into_iter()
            .filter(|light| light.id == arguments.id)
            .next(),
        Err(e) => {
    ...
    match arguments.action {
        ActionArguments::Switch => match light.switch(!light.on).await {
            Ok(_) => (),
            Err(e) => {
                println!("Unexpected Hue error {:?}.", e);
                return;
        ...
        ActionArguments::Rgb(color) => {
            match light
                .set_color_rgb(RGB8::new(color.r, color.g, color.b))
                .await
            {
                Ok(_) => (),
                Err(e) => {
        ...
        ActionArguments::Dimm(dimm) => match light.dimm(dimm.value).await {
            Ok(_) => (),
            Err(e) => {
                println!("Unexpected Hue error {:?}.", e);
                return;
