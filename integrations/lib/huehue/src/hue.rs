...
impl Hue {
    ...
    pub fn url(&self, path: &str) -> url::Url {
        Url::parse(format!("https://{}/{}", self.bridge.address.to_string(), path).as_str())
            .unwrap()
    }
    ...
    pub async fn lights(&self) -> Result<Lights, HueError> {
        ...
        if let Some(data) = response.data {
            return Ok(data
                .into_iter()
                .map(|datum| Light::new(self, datum))
                .collect());
        }
    ...
    pub async fn light_group_by_id(&self, id: String) -> Result<Light, HueError> {
        return self
            .light_resource_by_id("grouped_light".to_string(), id)
            .await;
    }
    ...
    async fn light_resource_by_id(&self, resource: String, id: String) -> Result<Light, HueError> {
        ...
        let response: GetLightsResponse =
            http::get_auth(self.application_key.clone().unwrap(), self.url(&url)).await?;
        println!("{:?}", response);
    ...
    pub async fn rooms(&self) -> Result<Rooms, HueError> {
        ...
        let response: GetRoomsResponse = http::get_auth(
            self.application_key.clone().unwrap(),
            self.url("clip/v2/resource/room"),
        )
        .await?;
    ...
    pub async fn devices(&self) -> Result<Devices, HueError> {
        ...
        if let Some(data) = response.data {
            return Ok(data
                .into_iter()
                .map(|datum| Device::new(self, datum))
                .collect());
        }
        if let Some(error) = response.error {
            return Err(HueError::from(error.r#type.clone()));
